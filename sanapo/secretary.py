...
class Secretary:
    def __init__(self, m_type, addr, logger, config):
        self.m_type = m_type
        self.addr = addr
        self.log = logger
        self._cfg = config
    ...
    def _system_msg_handler(self, frame: Frame):
        if frame.sys_type == SysType.UNIT_SHUTDOWN:
            self._on_unit_shutdown()
        elif frame.sys_type == SysType.UNIT_STOP:
            self._on_unit_stop()
        elif frame.sys_type == SysType.UNIT_SLEEP:
            self.module._is_paused = True
        elif frame.sys_type == SysType.UNIT_WAKEUP:
            self.module._is_paused = False

    def _on_unit_shutdown(self):
        self.log.inf(f"Unit {self.addr.name} starting shutdown sequence...")
        self.module.shutdown()
        # Если модуль пассивный, секретарь сам рапортует о готовности
        if self.m_type in [ModuleType.ZOMBIE, ModuleType.TICKABLE]:
            self._send_sys_to_kernel(SysType.UNIT_HALTED)
            self.stop()

    def _on_unit_stop(self):
        self.log.wrn(f"Unit {self.addr.name} received HARD STOP!")
        self.module.stop()
        self.stop()
    ...
    def step(self):
        """Проход для TICKABLE модулей."""
        if self._is_active and not self.module._is_paused:
            self.module.step()
...
class Secretary:
    ...
    def __init__(self,
        ...
        self._tick_rate: float = self._config.SECRETARY_TICK_TCT_DEFAULT
    ...
    def subscribe(self, cb: Callable, cmd: Enum = None, evt: Enum = None) -> None:
        ...
        if evt:
            ...
            p["evt_list"] = [evt]
        if p: self._send_sys(SysType.SUB, p)
    ...
    def unsubscribe(self, cmd: Enum = None, evt: Enum = None) -> None:
        ...
        if evt:
            ...
            p["evt_list"] = [evt]
        if p: self._send_sys(SysType.UNSUB, p)

    def configure_subscriptions(self, events: dict = None, commands: dict = None) -> None:
        p = {}
        if events:
            self._handlers_evt.update(events)
            p["evt_list"] = list(events.keys())
        if commands:
            self._handlers_cmd.update(commands)
            p["cmd_list"] = list(commands.keys())
        if p: self._send_sys(SysType.SUB_SETUP, p)
